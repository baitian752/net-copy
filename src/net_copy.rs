...
use crate::proxy::{Proxy, ProxyConsumer};
...
impl NetCopy {
  ...
  fn handle_send(&self, stream: TcpStream, file_name: &str, file_path: &Path, is_archive: bool) {
    ...
    if let Some(proxy) = &self.proxy {
      ...
      loop {
        ...
        if lines[0].trim() == "REQUEST" {
          ...
          self.handle_send(stream, file_name, &file_path, is_archive);
        }
      }
    } else {
      ...
      for stream in listener.incoming() {
        match stream {
          Ok(stream) => self.handle_send(stream, file_name, &file_path, is_archive),
          Err(e) => {
  ...
  fn handle_recv(&self, stream: TcpStream) {
    let key = self.key.clone();
    ...
    if let Some(proxy) = &self.proxy {
      ...
      loop {
        ...
        if lines[0].trim() == "REQUEST" {
          ...
          if let Err(e) = stream
            .write_all(format!("SEND {}\r\n\r\n", self.key).as_bytes())
            .and_then(|_| master_buf_stream.flush())
          {
            println!("Write to master stream failed: {}", e);
            continue;
          }
          self.handle_recv(stream);
        }
      }
    } else {
      let listener = TcpListener::bind(&self.socket)
      ...
      for stream in listener.incoming() {
        match stream {
          Ok(stream) => self.handle_recv(stream),
          Err(e) => {
            println!("Get incoming stream failed: {}", e);
            continue;
          }
